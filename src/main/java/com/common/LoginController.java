...
public class LoginController {
    ...
    public @ResponseBody ModelAndView login () {
        User command = myUserContext.getCurrentUser();
        if (command != null) {
            //user is already logged in, redirect to accounts page
            return new ModelAndView("redirect:/home");
        }

        return new ModelAndView("common/login", "command", new LoginCommand());
    }
    ...
    public @ResponseBody ModelAndView failedLogin () {
        ...
        return new ModelAndView("common/login", "command", command);
    }
